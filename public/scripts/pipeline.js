(function () {
  ...
  function initTitleLightning() {
    ...
    const mainPath =
      'M-20 35 58 39 91 24 118 53 154 41 176 68 219 50 247 78 286 55 321 64 350 30 387 59 431 44 456 73 501 51 534 82 574 58 607 69 642 37 678 66 719 48 748 79 791 55 825 72 861 43 897 68 938 52 970 70 1020 57';
    const paths = [
      {
        className: 'section-title-lightning-main section-title-lightning-glow',
        data: mainPath,
      },
      {
        className: 'section-title-lightning-main section-title-lightning-core',
        data: mainPath,
      },
      {
        className: 'section-title-lightning-branch section-title-lightning-branch-one section-title-lightning-glow',
        data: 'M247 78 220 91 191 86 166 98',
      },
      {
        className: 'section-title-lightning-branch section-title-lightning-branch-one section-title-lightning-core',
        data: 'M247 78 220 91 191 86 166 98',
      },
      {
        className: 'section-title-lightning-branch section-title-lightning-branch-two section-title-lightning-glow',
        data: 'M501 51 526 27 556 35 579 16',
      },
      {
        className: 'section-title-lightning-branch section-title-lightning-branch-two section-title-lightning-core',
        data: 'M501 51 526 27 556 35 579 16',
      },
      {
        className: 'section-title-lightning-branch section-title-lightning-branch-three section-title-lightning-glow',
        data: 'M791 55 765 31 735 38 706 18',
      },
      {
        className: 'section-title-lightning-branch section-title-lightning-branch-three section-title-lightning-core',
        data: 'M791 55 765 31 735 38 706 18',
      },
    ];
    const sparks = [
    ...
    titles.forEach((title) => {
      ...
      lightning.classList.add('section-title-lightning');
      lightning.setAttribute('viewBox', '0 0 1000 100');
      lightning.setAttribute('preserveAspectRatio', 'none');
      lightning.setAttribute('aria-hidden', 'true');
      lightning.setAttribute('focusable', 'false');

      paths.forEach(({ className, data }) => {
        const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        path.classList.add(...className.split(' '));
        path.setAttribute('d', data);
        path.setAttribute('pathLength', '1');
        lightning.append(path);
      });

      sparks.forEach(([x, y, radius, delay]) => {
        const spark = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        spark.classList.add('section-title-lightning-spark');
        spark.setAttribute('cx', String(x));
        spark.setAttribute('cy', String(y));
        spark.setAttribute('r', String(radius));
        spark.style.setProperty('--spark-delay', String(delay));
        lightning.append(spark);
      });

      title.append(lightning);
    });

    function play(title) {
      if (!(title instanceof HTMLElement) || document.hidden || motionPreference.matches) {
        return;
      }
      title.classList.remove('is-thundering');
      void title.offsetWidth;
      title.classList.add('is-thundering');
    }
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            ...
            entry.target.classList.remove('is-thundering');
          }
        });
      },
      { threshold: 0.55 },
    ...
    motionPreference.addEventListener('change', () => {
      if (motionPreference.matches) {
        titles.forEach((title) => title.classList.remove('is-thundering'));
      } else {
        visibleTitles.forEach(play);
      }
    });
